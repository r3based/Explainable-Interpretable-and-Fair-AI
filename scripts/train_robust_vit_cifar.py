...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Adversarially fine-tune a ViT CIFAR-10 classifier.")
    parser.add_argument("--data-dir", type=str, default="data")
    parser.add_argument("--model-name", type=str, default="vit_base_patch16_224")
    parser.add_argument("--checkpoint", type=str, default=None, help="Optional fine-tuned checkpoint to continue from.")
...
def main() -> None:
    ...
    if args.checkpoint:
        model = ViTCIFAR10Classifier.load_from_checkpoint(args.checkpoint, device=device)
        ...
        model.config.unfreeze_last_blocks = args.unfreeze_last_blocks
    else:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        robust_metrics = evaluate_under_attack(
            model=model,
            loader=val_loader,
            device=device,
            attack_config=attack_config,
        ...
        record = {
            ...
            "robust_validation": robust_metrics,
        ...
        print(
            ...
            f"clean_val_acc={clean_metrics['accuracy']:.4f} "
            f"robust_val_acc={robust_metrics['robust_accuracy']:.4f}"
        )

        latest_path = checkpoint_dir / f"{args.run_name}_latest.pt"
        model.save_checkpoint(
            latest_path,
            extra={"args": vars(args), "history": history, "best_robust_accuracy": best_robust_accuracy},
        )
        if robust_metrics["robust_accuracy"] > best_robust_accuracy:
            best_robust_accuracy = float(robust_metrics["robust_accuracy"])
